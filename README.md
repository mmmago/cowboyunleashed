...
The settings are not stored until you don't press "store" and "close", so if you just want to try some settings, apply them without storing. Locking the bike will dismiss them. Note that an way, **all changes are reversible**, as you can set everything to default.
